...
func UserActivityList(ver string) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		...
		favorite := StringToBool(ctx.Query("favorite"))
		act := model.NewUserActivity("")
		act.UserID = user.ID

		var session *xorm.Session
		if favorite {
			session = model.Where("user_activity.is_favorite = ?", favorite)
...
func UserActivityFavoriteUpdate(ver string) gin.HandlerFunc {
	return func(ctx *gin.Context) {
		user := model.GetUser(ctx)
		id := ctx.Param("id")
		...
		act.IsFavorite = StringToBool(favorite)
